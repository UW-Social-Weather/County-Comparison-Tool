...
us_health_all <- merge(us_health_all, SVI_grouping_data, by=c("FIPS","ST"), all.x= TRUE)
# us_health_all <- rename(us_health_all, FIPS = FIPS.x)
us_health_all <- rename(us_health_all, State = State.x)
us_health_all <- rename(us_health_all, NAME_2 = NAME_2.x)

us_health_all <- merge(us_health_all, pop_grouping, by=c("FIPS","ST"), all.x= TRUE)
us_health_all <- us_health_all %>% select(-State.y, -NAME_2.y, -ST_NUM, -LOCATION, -AREA_SQMI, -County_Name, -Population_2010, -RUCC_2013)
...
# us_health_counties1 <- rename(us_health_counties1, FIPS = FIPS.x)
us_health_states1 <- rename(us_health_states1, State = State.x)
us_health_states1 <- us_health_states1 %>% arrange(FIPS, year)

# add in population data 
us_health_counties1 <- merge(us_health_counties1, pop_grouping, by=c("FIPS","ST"), all.x=TRUE)
us_health_counties1 <- us_health_counties1 %>% select(-County_Name, -Population_2010, -RUCC_2013)
us_health_counties1 <- us_health_counties1 %>% arrange(FIPS, year)

health_vars <- sort(names(us_health_all)[6:29])
# all_vars <- sort(names(us_health_all)[6:ncol(us_health_all)])
health_vars <- health_vars[!grepl("-CI", health_vars)]
health_vars <- health_vars[!grepl("-Z", health_vars)]
