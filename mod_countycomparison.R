...
mod_healthdown <- function(input, output, session) {
  ...
  data <- reactive({
    ...
    if (my_leafdown$curr_map_level == 2) {
      ...
      us_health_counties_year <- subset(us_health_counties1, year == input$year, SVI_Group == input$SVI_Group)
      ...
      us_health_states_year <- subset(us_health_states1, year == input$year, SVI_Group == input$SVI_Group)
